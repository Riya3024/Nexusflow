...
const { calculateFreightEmissions } = require("./emissionsService");
...
async function buildShipmentPlan(origin, destination, weight, priority, budget) {
  ...
  const roadEmissions = calculateFreightEmissions({
    distanceKm: distance,
    weightKg: weight,
    mode: "road"
  });

  const airEmissions = calculateFreightEmissions({
    distanceKm: distance,
    weightKg: weight,
    mode: "air"
  });

  const seaEmissions = calculateFreightEmissions({
    distanceKm: distance,
    weightKg: weight,
    mode: "sea"
  });
  ...
  console.log("ORIGIN HAZARD:", JSON.stringify(originHazard, null, 2));
  console.log("DEST HAZARD:", JSON.stringify(destHazard, null, 2));
  ...
  const options = [
    {
      ...
      time: roadTime,
      risk: roadRisk,
      fuelLiters: roadEmissions.fuelLiters,
      co2Kg: roadEmissions.co2Kg,
      tonneKm: roadEmissions.tonneKm,
      co2KgDistanceBased: roadEmissions.co2KgDistanceBased
    },
    {
      mode: "Air",
      cost: airCost,
      time: airTime,
      risk: airRisk,
      fuelLiters: airEmissions.fuelLiters,
      co2Kg: airEmissions.co2Kg,
      tonneKm: airEmissions.tonneKm,
      co2KgDistanceBased: airEmissions.co2KgDistanceBased
    },
    {
      mode: "Sea",
      cost: seaCost,
      time: seaTime,
      risk: seaRisk,
      fuelLiters: seaEmissions.fuelLiters,
      co2Kg: seaEmissions.co2Kg,
      tonneKm: seaEmissions.tonneKm,
      co2KgDistanceBased: seaEmissions.co2KgDistanceBased
    }
  ];
  ...
  const recommended = options.reduce(
    (best, opt) => (scoreOption(opt) < scoreOption(best) ? opt : best),
    options[0]
  );
